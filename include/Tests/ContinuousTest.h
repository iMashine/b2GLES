...
public:
    ...
    void Step(Settings *settings)
    {
        ...
        if (b2_gjkCalls > 0) {
            sprintf(buffer, "gjk calls = %d, ave gjk iters = %3.1f, max gjk iters = %d",
        ...
        if (b2_toiCalls > 0) {
            sprintf(buffer, "toi calls = %d, ave [max] toi iters = %3.1f [%d]",
                    b2_toiCalls, b2_toiIters / float32(b2_toiCalls), b2_toiMaxRootIters);
            g_debugDraw.DrawString(m_painter, 5, m_textLine, buffer);
            m_textLine += DRAW_STRING_NEW_LINE;

            sprintf(buffer, "ave [max] toi root iters = %3.1f [%d]",
                    b2_toiRootIters / float32(b2_toiCalls), b2_toiMaxRootIters);
            g_debugDraw.DrawString(m_painter, 5, m_textLine, buffer);
            m_textLine += DRAW_STRING_NEW_LINE;

            sprintf(buffer, "ave [max] toi time = %.1f [%.1f] (microseconds)",
