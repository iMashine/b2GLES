...
public:
    ...
    {
        Test::Step(settings);
        g_debugDraw.DrawString(m_painter, 5, m_textLine, "Press (j) to toggle the rope joint.");
        m_textLine += DRAW_STRING_NEW_LINE;
        if (m_rope) {
            g_debugDraw.DrawString(m_painter, 5, m_textLine, "Rope ON");
        }
        else {
            g_debugDraw.DrawString(m_painter, 5, m_textLine, "Rope OFF");
        }
        m_textLine += DRAW_STRING_NEW_LINE;
    }
