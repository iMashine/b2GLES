...
public:
    ...
    void Step(Settings *settings)
    {
        ...
        Test::Step(settings);
        g_debugDraw.DrawString(m_painter, 5, m_textLine, "Press 1-6 to drop stuff, m to change the mode");
        m_textLine += DRAW_STRING_NEW_LINE;
        switch (m_mode) {
        case e_closest:
            g_debugDraw.DrawString(m_painter, 5, m_textLine, "Ray-cast mode: closest - find closest fixture along the ray");
            break;

        case e_any:
            g_debugDraw.DrawString(m_painter, 5, m_textLine, "Ray-cast mode: any - check for obstruction");
            break;

        case e_multiple:
            g_debugDraw.DrawString(m_painter, 5, m_textLine, "Ray-cast mode: multiple - gather multiple fixtures");
            break;
        }
